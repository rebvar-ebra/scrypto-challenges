...
blueprint! {
    ...
    impl Escrow {
        ...
        pub fn instantiate_escrow(
            ...
            assert(to_be_paid_by_party_1.validate().is_ok(),"First resource specifier is not valid");
            assert(to_be_paid_by_party_1.validate().is_ok(),"Second resource specifier is not valid");

            assert_ne!(
                to_be_paid_by_party_1, to_be_paid_by_party_2,
                 *The two resource specifiers can not equal one another.*);      
                 
            let party_1_obligation: EscrowObligation = EscroxObligation {
                 anount_to_pay: to_be_paid_by_party_1.clone(),
                amount_to_get: to_be_paid_by_party_2.clone()
            };
            

            let party_2_obligation: EscrosObligation = EscrosObligation {
